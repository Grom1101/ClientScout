  ...
  const RadioOption = ({ label, selected, onClick }: { label: string; selected: boolean; onClick: () => void }) => (
    <button
      ...
      style={{
        paddingLeft: '15px',
        paddingRight: '15px',
        backgroundColor: selected ? 'rgba(99,102,241,0.1)' : 'rgba(0,0,0,0.15)',
        border: selected ? '1px solid rgba(99,102,241,0.4)' : '1px solid rgba(255,255,255,0.04)',
      }}
    >
      ...
      <div
        ...
        style={{
          border: selected ? '5px solid #6366F1' : '1px solid rgba(255,255,255,0.2)',
          backgroundColor: selected ? '#fff' : 'transparent',
          boxShadow: selected ? '0 0 10px rgba(99,102,241,0.5)' : 'none',
        }}
      />
    </button>
  );
          ...
          <div className="grid grid-cols-3 gap-2">
            {PERIODICITY_OPTIONS.map((option) => (
              <button
                ...
                style={{
                  backgroundColor: draftPeriodicity === option.value ? 'rgba(99,102,241,0.24)' : 'rgba(255,255,255,0.04)',
                  color: draftPeriodicity === option.value ? '#FFFFFF' : '#94A3B8',
                  border: draftPeriodicity === option.value ? '1px solid rgba(129,140,248,0.45)' : '1px solid rgba(255,255,255,0.08)',
                }}
              >
                {option.label}
              </button>
